setwd("/home/baign/HPC/Project/PotatoTools/PhDNadia/Potato_samples_processing//Final_24_feb_2021_all_vcfs/SNP_array_list_final/tetraploids/recommended/LD_analysis/maf")
library(ggplot2)
...
head(pop_t)
pdf("MAF_all.pdf", height=5, width = 5)

#png("before_maf.png",width=6,height=6,units="in",res=1200)
hist(pop_t$MAF, main = "Minor Allele Frequency Distribution ", xlab = "Minor Allele Frequency",ylab="Number of SNPs", xlim = c(0.0, 0.5), col = "peachpuff")
dev.off()
####making boxplot per chromosomes https://r-coder.com/boxplot-r/
pdf("boxplot_all_maf_colored.pdf", height=5, width = 5)
boxplot(MAF ~ Chr, data = pop_t,col = rainbow(ncol(pop_t))) # Equivalent
dev.off()
...
#res
# Boxplot by group
pdf("MAF_all_chr.pdf", height=5, width = 5)
